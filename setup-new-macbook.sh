...
if [[ "$(id -u)" -eq 0 ]]; then
    ...
    echo "Homebrew refuses to run as root. Run as your normal user instead:" >&2
    echo "  ./setup-new-macbook.sh" >&2
    exit 1
fi
...
install_homebrew() {
    ...
    if command_exists brew; then
        ...
        print_info "Homebrew PATH will be configured by generate_zprofile"
    else
...
# =============================================================================
# Generate .zprofile (Homebrew shellenv)
# =============================================================================

generate_zprofile() {
    print_section "Generate .zprofile"

    local brew_prefix
    brew_prefix=$(get_homebrew_prefix)

    if ! command_exists brew; then
        print_info "Homebrew not installed — skipping .zprofile generation"
        return 0
    fi
    ...
    content+=$'# Generated by setup-new-macbook.sh — do not edit manually\n'
    content+=$'\n'
    content+=$'# Homebrew\n'
    content+="eval \"\$(${brew_prefix}/bin/brew shellenv)\""$'\n'

    # Back up existing .zprofile if different
    if [ -f "$zprofile" ]; then
        if printf '%s' "$content" | cmp -s - "$zprofile"; then
            print_success ".zprofile already up to date"
            return 0
        fi
        cp "$zprofile" "${zprofile}.bak"
        print_info "Backed up existing .zprofile to .zprofile.bak"
    fi

    printf '%s' "$content" > "$zprofile"
    print_success "Generated .zprofile (Homebrew at $brew_prefix)"
}
